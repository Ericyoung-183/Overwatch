...
For an active agenda, the prompt hook also blocks prose-only completion when the tracker still marks that item as discussing. The completion phrase and current-item reference must occur in the same non-interrogative clause; short labels and common wording such as “做完了”, “验收通过”, “没问题了”, or “结论已确定” are supported without treating “完成度”, “解决方案”, questions, or ordinary code-completion prose as agenda completion. Missing helper, status, or transcript identity surfaces an explicit warning instead of silently disabling this gate. A false transition detection can be dismissed explicitly with a non-empty reason and leaves an audit receipt.
...
- **Relocatable installation**: Both installers replace stale or duplicate managed Hook paths with one shell-quoted current path. Hook Python imports receive the install directory through the environment, so spaces and quotes remain safe. Install, shared-uninstall, and uninstall backup replacement reject symbolic-link configs and use atomic exchange/no-replace compare-and-swap. Configs and their existing `.backup` files roll back as one transaction. If an external write lands immediately after exchange or during rollback, Overwatch keeps it active or quarantined and preserves displaced bytes in a named recovery file. Cross-runtime rollback retains each config's own mode. A successful rollback of a newly created file intentionally leaves the removed managed bytes in a named `.overwatch-rollback.*.bak` quarantine so a late open-file write cannot be silently deleted.
